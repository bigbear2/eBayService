// Decompiled with JetBrains decompiler
// Type: eBay.Service.Core.Soap.CustomSecurityHeaderType
// Assembly: eBay.Service, Version=3.1131.0.0, Culture=neutral, PublicKeyToken=1d9d786a5932eaf0
// MVID: DB37CD86-03E4-4A03-BCFC-56AD7C99FC60
// Assembly location: D:\Sviluppo\Lazarus\Progetti\Agrozootecnica\Programmi\Gestione eBay\eBayRestApi\eBay.Service.dll
// XML documentation location: D:\Sviluppo\Lazarus\Progetti\Agrozootecnica\Programmi\Gestione eBay\eBayRestApi\eBay.Service.xml

using System;
using System.CodeDom.Compiler;
using System.ComponentModel;
using System.Diagnostics;
using System.Web.Services.Protocols;
using System.Xml.Serialization;


namespace eBay.Service.Core.Soap
{
  /// <remarks />
  [GeneratedCode("CodeGen05", "1.0.7142.28876")]
  [DebuggerStepThrough]
  [DesignerCategory("code")]
  [XmlType(Namespace = "urn:ebay:apis:eBLBaseComponents")]
  [XmlRoot("RequesterCredentials", Namespace = "urn:ebay:apis:eBLBaseComponents", IsNullable = false)]
  [Serializable]
  public class CustomSecurityHeaderType : SoapHeader
  {
    private string moAuthToken;
    private string meBayAuthToken;
    private string mHardExpirationWarning;
    private UserIdPasswordType mCredentials;
    private string mNotificationSignature;
    private XmlElementCollection mAny;

    /// <summary>
    /// 
    /// </summary>
    public string oAuthToken
    {
      get => this.moAuthToken;
      set => this.moAuthToken = value;
    }

    /// <summary>
    /// 
    /// </summary>
    public string eBayAuthToken
    {
      get => this.meBayAuthToken;
      set => this.meBayAuthToken = value;
    }

    /// <summary>
    /// 
    /// </summary>
    public string HardExpirationWarning
    {
      get => this.mHardExpirationWarning;
      set => this.mHardExpirationWarning = value;
    }

    /// <summary>
    /// 
    /// </summary>
    public UserIdPasswordType Credentials
    {
      get => this.mCredentials;
      set => this.mCredentials = value;
    }

    /// <summary>
    /// 
    /// </summary>
    public string NotificationSignature
    {
      get => this.mNotificationSignature;
      set => this.mNotificationSignature = value;
    }

    /// <summary>
    /// 
    /// </summary>
    [XmlAnyElement]
    public XmlElementCollection Any
    {
      get => this.mAny;
      set => this.mAny = value;
    }
  }
}
